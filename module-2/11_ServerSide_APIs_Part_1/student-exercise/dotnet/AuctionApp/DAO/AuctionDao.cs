...
namespace AuctionApp.DAO
{
    public class AuctionDao : IAuctionDao
    {
        ...
        {
            if (!auction.IsValid) return null;
        ...
        public List<Auction> SearchByTitle(string searchTerm) {
            return Auctions.Where(auction => auction.Title.Contains(searchTerm, StringComparison.CurrentCultureIgnoreCase)).ToList();
        }

        public List<Auction> SearchByPrice(double maxPrice) {
            return Auctions.Where(auction => auction.CurrentBid <= maxPrice).ToList();
        }
